...
//						newFrame[nFr][nPoint] =  Roi3D.scaleGlob(newFrame[nFr][nPoint],BigTraceData.globCal);
			...
			LineTrace3D newROI = addROIsegment(bt, segment_new,0, ( int ) firstLine.getLineThickness());
...
//				frameX2.add( new RealPoint(Roi3D.scaleGlobInv(pos,BigTraceData.globCal)));
			...
			addROIsegment(bt, frameX1,0, 2);
			addROIsegment(bt, frameX2,0, 2);
		}
		
//		ArrayList<double []> tangents = ((LineTrace3D) bt.roiManager.rois.get( 0 )).getJointSegmentTangentsResampled();
//		
//		//get all the points along the line with pixel sampling in scaled coords
//		ArrayList<RealPoint> segment = ((LineTrace3D) bt.roiManager.rois.get( 0 )).getJointSegmentResampled();
//		
//		//get a frame around line in SCALED
//		double [][][] rsVect =  Pipe3D.rotationMinimizingFrame(segment, tangents);
//		
...
//		double [] comp =  new double[3];
//		double [] addition = new double[3];
//		for(int nPoint = 0;nPoint<segment.size();nPoint++)
//		{
//			segment.get( nPoint ).localize( pos );
//			//pos = Roi3D.scaleGlobInv( pos, BigTraceData.globCal );
//			LinAlgHelpers.scale( rsVect[0][nPoint], 35*BigTraceData.dMinVoxelSize, addition );
//			LinAlgHelpers.add( addition, pos, comp );
//			frameX1.add( new RealPoint(Roi3D.scaleGlobInv(comp,BigTraceData.globCal)));
//			LinAlgHelpers.scale( rsVect[1][nPoint], 35*BigTraceData.dMinVoxelSize, addition );
//			LinAlgHelpers.add( addition, pos, comp );
//			frameX2.add( new RealPoint(Roi3D.scaleGlobInv(comp,BigTraceData.globCal)));
//
//		}
...
//		addROIsegment(bt, frameX2,0, 2);
	...
	public void generateSingleVolume(int nInd)
	{
		
		FinalInterval test = Intervals.expand( bt.roiManager.rois.get(nInd).getBoundingBox(),2);
		...
		int nRadius = (int) Math.round(0.5*bt.roiManager.rois.get(0).getLineThickness());
		...
		String sPathOutTif = "/home/eugene/Desktop/projects/BigTrace/BigTrace_progress/20240726_bending/snake/out/vol_T"+ String.format("%0"+String.valueOf(nFrames).length()+"d", nInd) +".tif";
	...
	public void generateSingleVolumeTemplate(int nInd)
	{
	...
	public void generateAllVolumes()
	{
		...
		{
			generateSingleVolumeTemplate(i);
	...
	{
		final ImagePlus imp = IJ.openImage( "/home/eugene/Desktop/projects/BigTrace/BigTrace_progress/20240726_bending/snake/clean_X_trace172232825_straight.tif" );
